...
RE_SIGNATURE = re.compile(r'\[\[(Wikipedista|Wikipedistka):[^|]+\|[^]]+\]\] \(\[\[Diskuse s (wikipedistou|wikipedistkou):[^|]+\|diskuse\]\]\) ([0-9]+)\. ([0-9]+)\. ([0-9]{4}), ([0-9]+):([0-9]+) \(CET|CEST')
	...
	for candidate in RE_SUBPAGE.finditer(page.text):
		...
		if voting is None:
			...
			if unusual_close is None or unusual_close.group(1).strip() == "standardní":
				...
				voting_start = datetime(int(voting_start_m.group(5)), int(voting_start_m.group(4)), int(voting_start_m.group(3)))
...
ANNOUNCES = [
		{'page':u'Šablona:OznámeníRC/NNČ', 'big_separator': "", 'big_big_big_tl': '<!-- Tato stránka je pravidelně aktualizována botem. Needitujte ji ručně, vaše změny by byly brzy přepsány. -->\n|- id="Sablona--OznameniRC__Oznameni_Nejlepsi-clanky"\n![[Wikipedie:Nominace nejlepších článků|Nejlepší články]]:\n| %s<noinclude>{{Dlouhodobě polozamčeno}}</noinclude>', 'big_big_tl': "<div>'''%(description)s''': %(data)s</div>", 'big_tl':"%s", 'small_tl':u"[[%(PREFIX)s/%(subpage_name)s|%(subpage_name)s]] (%(date)s)", 'separator':u' • ', 'empty': '<!-- momentálně tu nic není; tato stránka je pravidelně aktualizována botem, prosím, needitujte ji ručně --><noinclude>{{Dlouhodobě polozamčeno}}</noinclude>', 'inner_empty':u'<div></div>'},
		{'page':u'Wikipedie:Portál Wikipedie/Co se děje/Nominace nejlepších článků', 'big_separator': '\n', 'big_big_big_tl': '<!-- Tato stránka je pravidelně aktualizována botem. Needitujte ji ručně, vaše změny by byly brzy přepsány. -->\n%s', 'big_big_tl': "'''%(description)s''': %(data)s", 'big_tl':u'\n%s\n', 'small_tl':u"* [[%(PREFIX)s/%(subpage_name)s|%(subpage_name)s]] (%(date)s)", 'separator':u'\n', 'empty': '<!-- momentálně tu nic není; tato stránka je pravidelně aktualizována botem, needitujte ji, prosím, ručně -->', 'inner_empty':u'<div></div>'},
]
