...
ANNOUNCES = [
		{'page':u'Šablona:OznámeníRC/NNČ', 'big_separator': "", 'big_big_big_tl': '|- id="Sablona--OznameniRC__Oznameni_Nejlepsi-clanky"\n![[Wikipedie:Nominace nejlepších článků|Nejlepší články]]:\n| %s<noinclude>{{Dlouhodobě polozamčeno}}</noinclude>', 'big_big_tl': "<div>'''%(description)s''': %(data)s</div>", 'big_tl':"%s", 'small_tl':u"[[%(PREFIX)s/%(subpage_name)s|%(subpage_name)s]] (%(date)s)", 'separator':u' • ', 'empty': '<!-- momentálně tu nic není --><noinclude>{{Dlouhodobě polozamčeno}}</noinclude>', 'inner_empty':u'<div></div>'},
