...
sql = 'select count(*), pl_title from pagelinks where pl_namespace=0 and pl_title in (select page_title from page where page_id in (select rd_from from redirect) and page_namespace=0) group by pl_title order by count(*) desc, pl_title limit 100;'
