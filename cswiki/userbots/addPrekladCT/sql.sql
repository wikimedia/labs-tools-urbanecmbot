...
and rev_page not in
(select tl_from from templatelinks join linktarget on lt_id=tl_target_id where lt_title="Překlad")
and page_namespace=0
/*and rev_parent_id=0*/
and rev_page not in (select tl_from from templatelinks join linktarget on lt_id=tl_target_id where lt_title like "Rozcestník%")
